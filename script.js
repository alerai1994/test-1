...
async function showInfo(id){
    ...
    try{
        // CoinMarketCap API pubblica (usiamo API gratuita, demo)
        const symbolMap = {btcusdt:"BTC",ethusdt:"ETH",bnbusdt:"BNB",solusdt:"SOL",rayusdt:"RAY",oreusdt:"ORE"};
        const symbol = symbolMap[id];
        const url = `https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&ids=${symbol.toLowerCase()}`;
        ...
        if(data[0]){
            ...
            `;
            cmcLink.href = `https://coinmarketcap.com/currencies/${symbol.toLowerCase()}/`;
